...
class S3Helper {
     ...
     */
    deleteBucket(name, expectedBucketOwner) {
        let params = {
            Bucket: name
        };
     ...
     */
    getBucketPermissions(name) {
        return new Promise((resolve, reject) => {
            this.s3.getBucketAcl({ Bucket: name }, (err, data) => {
                if (err)
                    reject(err);
                else
                    resolve(data);
     ...
     */
    listObjects(bucket, prefix = null) {
        return new Promise((resolve, reject) => {
            let params = {
                Bucket: bucket
            };
            ...
            this.s3.listObjectsV2(params, (err, list) => {
                if (err)
                    reject(err);
                else {
    ...
    putData(bucket, folder, key, data) {
        ...
        return new Promise((resolve, reject) => {
            this.putBuffer(bucket, folder, key, Buffer.from(data, 'binary'))
                .then(r => resolve(r))
    ...
    put(bucket, folder, key, stream) {
        return this.putBuffer(bucket, folder, key, stream);
    }
    ...
    delete(bucket, file) {
        return new Promise((resolve, reject) => {
            this.s3.deleteObject({ Bucket: bucket, Key: file }, (err, data) => {
                if (err)
                    reject(err);
                else
                    resolve(data);
    ...
    deleteFolder(bucket, folder) {
        return new Promise((resolve, reject) => {
            this.s3.listObjectsV2({ Bucket: bucket, Prefix: folder }, (err, data) => {
                if (err)
                    reject(err);
                else {
    ...
    createFolder(bucket, folder) {
        return new Promise((resolve, reject) => {
            this.s3.listObjectsV2({ Bucket: bucket }, (err, exists) => {
                if (err)
                    reject(err);
                else {
                    if (!exists.Contents.length && folder !== null) {
                        this.s3.putObject({ Bucket: bucket, Key: `${folder}/`, Body: '' }, (err, data) => {
                            if (err)
                                reject(err);
                            else
                                resolve(true);
