...
module.exports = (data) => {
    ...
    if (!isNaN(args[0]) && args[0] > 1)
        ...
        return new Output().makeError('Cannot roll a d1.').reject();

    if (!isNaN(args[1]) && args[1] >= 1 && args[1] <= 50)
        count = args[1];
    else if (args[1] > 50)
        return new Output().makeError('Cannot roll more than 50 times at once.').reject();
    ...
    return new Output(value).setValues(...rolls).resolve();
};