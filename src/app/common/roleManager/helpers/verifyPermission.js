    ...
    return new Promise(async function(resolve, reject) {
        ...
        .then(config => {
            ...
            .then(user => {
                ...
                if (user.data.permissions.botAdmin) resolve({status: true, reason: 'botAdmin'});
                if (user.data.permissions.botMod && permissionLevel < 4) resolve({status: true, reason: 'botMod'});
                if (user.data.permissions.botHelper && permissionLevel < 3) resolve({status: true, reason: 'botHelper'});
