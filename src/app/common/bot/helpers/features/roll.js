...
module.exports = (args) => {
    return new Promise((resolve, reject) => {
        ...
        if (!isNaN(args[0]) && args[0] > 1)
            ...
            reject(commandFormat.error([null], ["Cannot roll a d1."]));

        if (!isNaN(args[1]) && args[1] >= 1 && args[1] <= 50)
            count = args[1];
        else if (args[1] > 50)
        reject(commandFormat.error([null], ["Cannot roll more than 50 times at once."]));
        ...
        resolve(commandFormat.valid(rolls, [value]));
    });
}