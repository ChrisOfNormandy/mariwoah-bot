class NoiseGen {
    XScale = 0.02;
    YScale = 0.02;
    ZScale = 0.02;
    Octaves = 1;

    Scale(value) {
        this.XScale = value;
        this.YScale = value;
    }

    GetNoise(x, y, z) {
        return (Octaves > 1) 
            ? new Noise().GetOctaveNoise(x * this.XScale, y * this.YScale, z * this.ZScale, this.Octaves)
            : new Noise().GetNoise(x * this.XScale, y * this.YScale, z * this.ZScale);
    }

    constructor(pXScale = 0.2, pYScale = 0.2, pOctaves = 1) {
        this.XScale = pXScale;
        this.YScale = pYScale;
        this.Octaves = pOctaves;
    }
}

// Simplex noise in 3D
class Noise {
    // Inner class to speed up gradient computations
    // (array access is a lot slower than member access)
    Grad(x, y, z) {
        return { x, y, z, w: 0 }
    }

    grad3 = [
        this.Grad(1, 1, 0), this.Grad(-1, 1, 0), this.Grad(1, -1, 0), this.Grad(-1, -1, 0),
        this.Grad(1, 0, 1), this.Grad(-1, 0, 1), this.Grad(1, 0, -1), this.Grad(-1, 0, -1),
        this.Grad(0, 1, 1), this.Grad(0, -1, 1), this.Grad(0, 1, -1), this.Grad(0, -1, -1)
    ];

    p = [
        151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23, 190, 6, 148,
        247, 120, 234, 75, 0, 26, 197, 62, 94, 252, 219, 203, 117, 35, 11, 32, 57, 177, 33, 88, 237, 149, 56, 87, 174, 20, 125, 136, 171, 168, 68, 175,
        74, 165, 71, 134, 139, 48, 27, 166, 77, 146, 158, 231, 83, 111, 229, 122, 60, 211, 133, 230, 220, 105, 92, 41, 55, 46, 245, 40, 244, 102, 143, 54,
        65, 25, 63, 161, 1, 216, 80, 73, 209, 76, 132, 187, 208, 89, 18, 169, 200, 196, 135, 130, 116, 188, 159, 86, 164, 100, 109, 198, 173, 186, 3, 64,
        52, 217, 226, 250, 124, 123, 5, 202, 38, 147, 118, 126, 255, 82, 85, 212, 207, 206, 59, 227, 47, 16, 58, 17, 182, 189, 28, 42, 223, 183, 170, 213,
        119, 248, 152, 2, 44, 154, 163, 70, 221, 153, 101, 155, 167, 43, 172, 9, 129, 22, 39, 253, 19, 98, 108, 110, 79, 113, 224, 232, 178, 185, 112, 104,
        218, 246, 97, 228, 251, 34, 242, 193, 238, 210, 144, 12, 191, 179, 162, 241, 81, 51, 145, 235, 249, 14, 239, 107, 49, 192, 214, 31, 181, 199, 106, 157,
        184, 84, 204, 176, 115, 121, 50, 45, 127, 4, 150, 254, 138, 236, 205, 93, 222, 114, 67, 29, 24, 72, 243, 141, 128, 195, 78, 66, 215, 61, 156, 180
    ];

    // To remove the need for index wrapping, the permutation table length
    perm = new Array(512);
    permMod12 = new Array(512);

    // Skewing and unskewing factors for 2, 3, and 4 dimensions
    F3 = 1.0 / 3.0;
    G3 = 1.0 / 6.0;

    // This method is a *lot* faster than using (int)Math.floor(x)
    fastfloor(x) {
        let xi = Math.floor(x);
        return x < xi ? xi - 1 : xi;
    }

    dot(grad, x, y, z) {
        return grad.x * x + grad.y * y + grad.z * z;
    }

    // 3D simplex noise
    GetNoise(xin, yin, zin) {
        let n0, n1, n2, n3; // Noise contributions from the four corners
        // Skew the input space to determine which simplex cell we're in
        let s = (xin + yin + zin) * this.F3; // Very nice and simple skew factor for 3D
        let i = this.fastfloor(xin + s);
        let j = this.fastfloor(yin + s);
        let k = this.fastfloor(zin + s);
        let t = (i + j + k) * this.G3;
        let X0 = i - t; // Unskew the cell origin back to (x,y,z) space
        let Y0 = j - t;
        let Z0 = k - t;
        let x0 = xin - X0; // The x,y,z distances from the cell origin
        let y0 = yin - Y0;
        let z0 = zin - Z0;
        // For the 3D case, the simplex shape is a slightly irregular tetrahedron.
        // Determine which simplex we are in.
        let i1, j1, k1; // Offsets for second corner of simplex in (i,j,k) coords
        let i2, j2, k2; // Offsets for third corner of simplex in (i,j,k) coords
        if (x0 >= y0) {
            if (y0 >= z0) {
                i1 = 1; j1 = 0; k1 = 0; i2 = 1; j2 = 1; k2 = 0;
            } // X Y Z order
            else if (x0 >= z0) {
                i1 = 1; j1 = 0; k1 = 0; i2 = 1; j2 = 0; k2 = 1;
            } // X Z Y order
            else {
                i1 = 0; j1 = 0; k1 = 1; i2 = 1; j2 = 0; k2 = 1;
            } // Z X Y order
        }
        else { // x0<y0
            if (y0 < z0) {
                i1 = 0; j1 = 0; k1 = 1; i2 = 0; j2 = 1; k2 = 1;
            } // Z Y X order
            else if (x0 < z0) {
                i1 = 0; j1 = 1; k1 = 0; i2 = 0; j2 = 1; k2 = 1;
            } // Y Z X order
            else {
                i1 = 0; j1 = 1; k1 = 0; i2 = 1; j2 = 1; k2 = 0;
            } // Y X Z order
        }
        // A step of (1,0,0) in (i,j,k) means a step of (1-c,-c,-c) in (x,y,z),
        // a step of (0,1,0) in (i,j,k) means a step of (-c,1-c,-c) in (x,y,z), and
        // a step of (0,0,1) in (i,j,k) means a step of (-c,-c,1-c) in (x,y,z), where
        // c = 1/6.
        let x1 = x0 - i1 + this.G3; // Offsets for second corner in (x,y,z) coords
        let y1 = y0 - j1 + this.G3;
        let z1 = z0 - k1 + this.G3;
        let x2 = x0 - i2 + 2.0 * this.G3; // Offsets for third corner in (x,y,z) coords
        let y2 = y0 - j2 + 2.0 * this.G3;
        let z2 = z0 - k2 + 2.0 * this.G3;
        let x3 = x0 - 1.0 + 3.0 * this.G3; // Offsets for last corner in (x,y,z) coords
        let y3 = y0 - 1.0 + 3.0 * this.G3;
        let z3 = z0 - 1.0 + 3.0 * this.G3;
        // Work out the hashed gradient indices of the four simplex corners
        let ii = i & 255;
        let jj = j & 255;
        let kk = k & 255;
        let gi0 = this.permMod12[ii + this.perm[jj + this.perm[kk]]];
        let gi1 = this.permMod12[ii + i1 + this.perm[jj + j1 + this.perm[kk + k1]]];
        let gi2 = this.permMod12[ii + i2 + this.perm[jj + j2 + this.perm[kk + k2]]];
        let gi3 = this.permMod12[ii + 1 + this.perm[jj + 1 + this.perm[kk + 1]]];
        // Calculate the contribution from the four corners
        let t0 = 0.6 - x0 * x0 - y0 * y0 - z0 * z0; // change to 0.5 if you want
        if (t0 < 0)
            n0 = 0.0;
        else {
            t0 *= t0;
            n0 = t0 * t0 * this.dot(this.grad3[gi0], x0, y0, z0);
        }
        let t1 = 0.6 - x1 * x1 - y1 * y1 - z1 * z1; // change to 0.5 if you want
        if (t1 < 0)
            n1 = 0.0;
        else {
            t1 *= t1;
            n1 = t1 * t1 * this.dot(this.grad3[gi1], x1, y1, z1);
        }
        let t2 = 0.6 - x2 * x2 - y2 * y2 - z2 * z2; // change to 0.5 if you want
        if (t2 < 0)
            n2 = 0.0;
        else {
            t2 *= t2;
            n2 = t2 * t2 * this.dot(this.grad3[gi2], x2, y2, z2);
        }
        let t3 = 0.6 - x3 * x3 - y3 * y3 - z3 * z3; // change to 0.5 if you want
        if (t3 < 0)
            n3 = 0.0;
        else {
            t3 *= t3;
            n3 = t3 * t3 * this.dot(this.grad3[gi3], x3, y3, z3);
        }
        // Add contributions from each corner to get the final noise value.
        // The result is scaled to stay just inside [-1,1] (now [0, 1])
        return (32.0 * (n0 + n1 + n2 + n3) + 1) * 0.5; // change to 76.0 if you want
    }

    // get multiple octaves of noise at once
    GetOctaveNoise(pX, pY, pZ, pOctaves) {
        let value = 0;
        let divisor = 0;
        let currentHalf = 0;
        let current = 0;

        for (let i = 0; i < pOctaves; i++) {
            currentHalf = Math.pow(0.5, i);
            current = Math.pow(2, i);
            value += this.GetNoise(pX * current, pY * current, pZ) * currentHalf;
            divisor += currentHalf;
        }

        return value / divisor;
    }

    constructor() {
        for (let i = 0; i < 512; i++) {
            this.perm[i] = this.p[i & 255];
            this.permMod12[i] = this.perm[i] % 12;
        }
    }
}

module.exports = {
    noiseGen(pXScale, pYScale, pOctaves) { return new NoiseGen(pXScale, pYScale, pOctaves) },
    noise() { return new Noise() },
    get(x, y, z, frequency, height, power) {
        //frequency = how many peaks will generate.
        //height = multiplier to the values generated.
        //power = stretch downwards. 0.01 = flat, 10 = low flat with skinny peaks

        let rValue = new Noise().GetNoise(x * frequency, y * frequency, z * frequency);
        rValue *= height;

        if (power != 0)
            rValue = Math.pow(rValue, power);

        return rValue;
    }
}