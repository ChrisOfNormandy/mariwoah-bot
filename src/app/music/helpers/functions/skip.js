...
module.exports = function (message) {
    ...
    if (!queue.has(message.guild.id) || !queue.get(message.guild.id).active)
        return Promise.reject(commandFormat.error([], [chatFormat.response.music.skip.no_queue()]));

    queue.get(message.guild.id).connection.dispatcher.end();
    return (queue.get(message.guild.id).songs.length > 1)
        ? Promise.resolve(commandFormat.valid([], [getEmbedSongInfo.single('Now playing...', queue.get(message.guild.id), 1)]))
        : Promise.resolve(commandFormat.valid([], [chatFormat.response.music.skip.plain()]));
}