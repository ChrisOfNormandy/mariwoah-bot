...
const itemstack = require('../inventory/itemstack');
const levelup = require('./levelup');

module.exports = function(userID, action, argsObject) {
    if (!statsMap.map.has(userID)) return;
    let user = statsMap.map.get(userID);

    switch (action) {
        case 'pay': {
            if (!argsObject.amount) return;
        ...
        case 'give': {
            ...
            switch (argsObject.category) {
                case 'items': {
                    ...
                    if (flag) user.inventories.items.push(argsObject);
                    break;
                }
                case 'fish': {
        ...
        case 'addCatch': {
            if (!argsObject.game || !argsObject.amount || !argsObject.flag) return;
            user.stats.games[argsObject.game][argsObject.flag] += Number(argsObject.amount);
            return;
        }
        case 'gain': {
            if (!argsObject.amount || !argsObject.game) return;
        ...
        case 'levelup': {
            if (!argsObject.game || !argsObject.message) return;
            let val = levelup.set(user, argsObject.game);
            if (!val) return;
            user = val;
